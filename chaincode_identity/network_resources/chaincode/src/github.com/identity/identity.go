...
func (t *IdentityData) initIdentity(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	objectType := "identity"
	identity := &identity{objectType, identityName, address, itype, rootID, timestamp}
...
func (t *IdentityData) quaryIdentityByName(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	name = args[0]
	valAsbytes, err := stub.GetState(name) //get the identity from chaincode state
	if err != nil {
...
func (t *IdentityData) delete(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	valAsbytes, err := stub.GetState(identityName) //get the identity from chaincode state
	if err != nil {
	...
	err = stub.DelState(identityName) //remove the identityidentity from chaincode state
	if err != nil {
